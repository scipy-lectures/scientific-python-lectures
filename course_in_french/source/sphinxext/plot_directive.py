...
def makefig(fullpath, outdir):
    ...
    pickle_filename = os.path.join(outdir, '%s.pkl' % basename)
    if not os.path.exists(pickle_filename):
        all_exists = False
    else:
        figure_basenames = pickle.load(file(pickle_filename))
        for this_basename in figure_basenames:
    ...
    figure_basenames = list()
    for fig_num in plt._pylab_helpers.Gcf.figs:
        plt.figure(fig_num)
        if len(plt._pylab_helpers.Gcf.figs.keys()) > 1:
            this_basename = '%s_%i' % (basename, fig_num)
        else:
            this_basename = basename
        figure_basenames.append(this_basename)
        for format, dpi in formats:
            outname = os.path.join(outdir, '%s.%s' % (this_basename, format))
...
def run(arguments, options, state_machine, lineno):
    ...
    figure_basenames = makefig(os.path.join(srcdir, reference), srcdir)
    ...
    if options.pop('hide-links', False):
        this_template = template_links + template
    else:
        this_template = template

    has_align = ('align' in options)
